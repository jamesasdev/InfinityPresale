...
const Balances: React.FC = () => {
  ...
  const handledeposit = async () => {
    ...
    setNum(depositnum);
      setHardCap((await presaleContract.methods.hardCapEthAmount().call())/1e18);
    return depositnum
  }
  handledeposit();
  useEffect(() => {
          ...
          <div style={{ flex: 1, }}>
            <div style={{display: 'flex', placeContent:'center'}}>
              ...
              <Value
                value={depositAmount}
              />
              <Label text={" BNB / "+hardCap.toString()+" BNB"} />
            </div>
            <div className={classes.root}>
              <PrettoSlider className='' key={`PrettoSlider-${depositAmount}`}
                valueLabelDisplay="off" defaultValue={depositAmount} min={0} max={hardCap}
